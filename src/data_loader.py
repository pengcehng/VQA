...
class VideoDataset_images_with_motion_features(data.Dataset):
    ...
    def __init__(self, data_dir, data_dir_3D, filename_path, transform, database_name, crop_size, feature_type):
        ...
        elif database_name == 'LSVQ_train' or database_name == 'LSVQ_test' or database_name == 'LSVQ_test_1080p':
            column_names = ['name', 'mos']
            dataInfo = pd.read_csv(filename_path, header=0, sep=',', names=column_names, usecols=column_names,
                                   encoding="utf-8-sig")
    ...
    def __getitem__(self, idx):
        if self.database_name == 'KoNViD-1k' or self.database_name == 'youtube_ugc':
...
class VideoDataset_NR_LSVQ_SlowFast_feature(data.Dataset):
    ...
    def __init__(self, data_dir, filename_path, transform, resize, is_test_1080p=False, column_names=['name', 'mos']):
