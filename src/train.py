...
def main(config):
    ...
    trainset = VideoDataset_images_with_motion_features(videos_dir, feature_dir, datainfo_train, transformations_train,
                                                        'LSVQ_train', config.crop_size, 'SlowFast')
    testset = VideoDataset_images_with_motion_features(videos_dir, feature_dir, datainfo_test, transformations_test,
                                                       'LSVQ_test', config.crop_size, 'SlowFast')
    testset_1080p = VideoDataset_images_with_motion_features(videos_dir, feature_dir, datainfo_test_1080p,
                                                             transformations_test, 'LSVQ_test_1080p', config.crop_size,
                                                             'SlowFast')
    ...
    for epoch in range(config.epochs):
        ...
        with torch.no_grad():
            ...
            print(
                'Epoch {} completed. The result on the test dataset: SRCC: {:.4f}, KRCC: {:.4f}, PLCC: {:.4f}, and RMSE: {:.4f}'.format(
                    epoch + 1, \
            ...
            print(
                'Epoch {} completed. The result on the test_1080p dataset: SRCC: {:.4f}, KRCC: {:.4f}, PLCC: {:.4f}, and RMSE: {:.4f}'.format(
                    epoch + 1, \
    ...
    print(
        'The best training result on the test_1080p dataset SRCC: {:.4f}, KRCC: {:.4f}, PLCC: {:.4f}, and RMSE: {:.4f}'.format( \
...
if __name__ == '__main__':
    ...
    parser.add_argument('--train_batch_size', type=int, default=1)  # 进一步减小批量大小
    parser.add_argument('--num_workers', type=int, default=2)  # 减少工作线程
    parser.add_argument('--resize', type=int, default=256)  # 进一步减小图像分辨率
    parser.add_argument('--crop_size', type=int, default=256)  # 减小裁剪大小
